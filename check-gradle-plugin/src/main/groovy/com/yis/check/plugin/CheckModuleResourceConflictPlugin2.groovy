 ...
 * 检测资源冲突
 */
class CheckModuleResourceConflictPlugin2 implements Plugin<Project> {

    /**
     * 存放所有的资源文件，会用这个容器中的数据判断是否有资源冲突
     */
    private Map<String, BaseResource> mResourceMap
     ...
     * value ：冲突的资源文件信息
     */
    private Map<String, List<BaseResource>> mConflictResourceMap
            ...
            variants.forEach { BaseVariantImpl variant ->
                ...
                thisTask.doLast {
                    ...
                    template = template.replaceAll("#File_Resouce_Conflicts#",
                            mGson.toJson(fileResourceList))
                    template = template.replaceAll("#Value_Resouce_Conflicts#",
                            mGson.toJson(valueResourceList))
     ...
     * @param resource
     */
    void recordResource(BaseResource resource) {
     ...
     * @return
     */
    BaseResource findAndRecordValueResource(File file) {
    ...
    void findAndRecordFileResource(File file) {
        ...
        println("path==" + file.path + "name == " + file.parentFile.name + "MD5==" + resource.md5)
    }
